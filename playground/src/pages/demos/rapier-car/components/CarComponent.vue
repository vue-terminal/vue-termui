...
const downward = new ThreeVector3(0, -1, 0)
...
function addWheel(controller: DynamicRayCastVehicleController, index: number, pos: Vector3Like) {
...
function updateChassisMeasures() {
  ...
  upsideDown.ratio = upward.dot(downward) * 0.5 + 0.5
...
function updateCarControl(dt = SIM_DT) {
  ...
  if (xzSpeed > 0.5 && ((throttle > 0 && !goingForward) || (throttle < 0 && goingForward))) {
    brake = REVERSE_BRAKE
    engineForce = 0
  }
  ...
  // Frame-rate-independent smoothing: STEER_RESPONSE is the per-frame factor
  // tuned at 1/60, re-based to real dt so steering feel is consistent at any fps
  const steerT = 1 - (1 - STEER_RESPONSE) ** (dt / SIM_DT)
  const steering = MathUtils.lerp(currentSteering, targetSteer, steerT)

  controller.setWheelSteering(0, steering)
  controller.setWheelSteering(1, steering)

  const sideFriction = movement.slide ? SLIDE_SIDE_FRICTION_STIFFNESS : SIDE_FRICTION_STIFFNESS
...
watch(
  ...
  ([car, chassis]) => {
    ...
    if (!wheelModels.value.length && wheelGroup) {
      ...
      // assign, not push: a shallowRef only re-renders the v-for on replacement
      wheelModels.value = [...wheelMounts]
    }

    if (chassisModel.value && wheelModels.value.length) {
      initVehicle(chassis)
    }
  },
  { immediate: true },
...
<template>
  <RigidBody ref="chassisRef" :collider="false" :linear-damping="0.15" :angular-damping="0.4">
